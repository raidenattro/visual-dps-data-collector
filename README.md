...
## 机位 OCR（与骨架采集同一 Python 环境）

在**同一 conda/venv** 中安装 CPU 版 Paddle（不要装 `paddlepaddle-gpu`），与 `onnxruntime-gpu` 采集可共存：

```powershell
conda activate visual-dps-datacollect
pip install -r requirements-ocr.txt
pip install -r requirements.txt
# 或: .\scripts\repair_env_after_ocr.ps1
```

装 OCR 后若 pip 提示 **`rtmlib requires onnxruntime`**：不是 OCR 坏了，是 Paddle 安装过程暂时卸掉了 `onnxruntime-gpu`。**再执行一次** `pip install -r requirements.txt` 或 `repair_env_after_ocr.ps1` 即可，骨架采集仍用 GPU。
