# RTMPose-M alarm_min=15 碰撞段 displacement × disp/fc 分布
...
|------|------|------|--------------|---------|
| 正确检测 | 与**已检出**标真段重叠的碰撞段 | 144 | P50=9.49 | P50=0.38 |
| 漏报 | 与**漏报**标真段重叠的碰撞段 | 74 | P50=8.30 | P50=1.14 |
| 误报 | 与 alarm_min=15 **误报告警**重叠、且不优先归标真段 | 14 | P50=41.91 | P50=1.71 |
...
| 误报 FP | 56 | 50 | **-6** |
| 漏报 FN | 41 | 42 | +1 |
| 召回 recall | 75.6% | 75.0% | -0.6% |

### 误报碰撞段（散点图红点）

- 误报重叠碰撞段 **14** 条；其中 `disp/fc > 2.5`：**4** 条（段级不通过，关联告警易被抑制）
- `disp/fc ≤ 2.5` 仍通过：**10** 条（段过滤后仍可能留下误报）
- **系统级误报减少 6 次**（56 → 50）
